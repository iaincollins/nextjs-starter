...
module.exports = function(app, server, options) {
  ...
  const serverUrl = (options && options.serverUrl) ? options.mailserver : null

  // Mailserver (defaults to sending from localhost)
  const mailserver = (options && options.mailserver) ? options.mailserver : null
  ...
  server.post(path+'/signin', (req, res) => {
    // @TODO Create verification token save it to database
    const emailVerificationToken = "ABC-123"
    const verificationUrl = (serverUrl || "http://"+req.headers.host)+'/auth/signin/'+emailVerificationToken
  ...
  server.get(path+'/signin/:emailToken', (req, res) => {
    // @TODO Verify emailToken in databases and log the user in if valid
